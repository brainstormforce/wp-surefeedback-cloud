...
class AuthMiddleware extends Middleware
{
    ...
    public function handle(WP_REST_Request $request, callable $next)
    {
        ...
        if (!is_user_logged_in()) {
            $this->log('auth_failed', 'User not logged in', [
        ...
        if (!$this->hasRequiredCapabilities($request)) {
            $this->log('auth_insufficient_permissions', 'User lacks required permissions', [
        ...
        if ($this->requiresNonceVerification($request)) {
            if (!$this->verifyNonce($request)) {
                $this->log('auth_invalid_nonce', 'Invalid or missing nonce', [
                    'user_id' => $this->getCurrentUserId(),
                    'endpoint' => $request->get_route()
                ]);
        ...
        $this->log('auth_success', 'Authentication successful', [
            'user_id' => $this->getCurrentUserId(),
            'endpoint' => $request->get_route()
        ]);
